 ...
 *
 * This module must not call dashboard `/api/shopify/*` routes. Those routes
 * write `shopify.accessToken` on the dashboard Store document. Sibling
 * issue #15 turns on the live redirect once the merchant lands back here
 * after approval (backend #153).
 *
 * Expected backend contract:
 * - POST /shopify/oauth/connect { shop, returnTo } -> { data: { authorizationUrl } }
 ...
 * `returnTo` is included so backend #153 can send the merchant back to the wizard.
 */
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ...
        error: message ?? 'Shopify could not be opened. Try again.',
      };
    }
