...
- Merchant onboarding: token creation, token expiry/revocation, email delivery if enabled, signup token validation, store creation, first-user role, post-signup login, and the `/dashboard/onboarding` steps. Connect Shopify opens the backend authorize URL. A return with `shopify=connected` shows success, sync progress, and the product count or "Not synced yet", and Continue to Brand stays available. A return with `shopify=error&reason=` shows plain copy and Reconnect Shopify. Refreshing does not repeat that banner. Branding save and the smart-default preview still apply. Build my app stays disabled until `GET /api/v1/shopify/sync` reports `eligibleForBuild` and Shopify is connected. Confirm the wizard did not write a Shopify access token.
- Branding/theme configuration: logo upload/remove, current branding fetch, and any mobile-consumed theme fields. On the brand step, an app icon and splash upload, show on the phone before save, and still show after reload. The screen must not ask for an API key or a Shopify access token.
...
- Build/status workflow: onboarding preview shows a smart-default home. On the ready step, an ineligible store cannot submit and sees Reconnect Shopify or Sync again. Settings shows the same one control: Sync again when the catalog is connected but not eligible, Reconnect Shopify when the store is disconnected. The card shows the last sync time, the product count, or the short failure, including `webhookRegistrationError` when status includes it. It does not ask for an Admin API key. Try again also refreshes Shopify connection status. An expired access token on that screen refreshes and retries once. An eligible store can request Android, iOS, or both and sees per-platform status, including Waiting on Apple for iOS. The screen must not show a build log or an EAS id. Also verify the existing app-builder homescreen preview. `/dashboard/admin/build-requests` is for platform operators only. A store owner must see the empty 403 state, not another store's access note. Grant ops on the backend (`isPlatformOperator` or `PLATFORM_OPS_EMAILS`) before expecting the queue to load. The dashboard has no setting for that allowlist.
