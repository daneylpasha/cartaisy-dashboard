...
- The guided wizard at `app/dashboard/onboarding/page.tsx` is the post-signup path: Connect Shopify, confirm brand, preview a starting home, then a ready-for-build step. It does not replace invite signup and it does not require the home module editor.
- Connect is isolated in `lib/onboarding/shopifyConnect.ts`. It is written to call backend Shopify routes only. `liveRedirectEnabled` is false until dashboard #15, because the current backend OAuth callback responds with JSON instead of returning the merchant to the wizard. This wizard does not write `shopify.accessToken` in dashboard Mongo.
...
- Store setup can still continue through dashboard settings: Shopify connection, store info/settings, logo upload, sync status, plan/usage, and app-builder pages. The settings Shopify card is the older flow and can still persist a dashboard access token. The new wizard does not use it.
...
- Live Shopify connect from the wizard waits on dashboard #15 and backend #153. Until then a merchant can continue to branding with a sync warning. Build stays off until sync succeeded, including when the sync status API cannot be read.
