...
- The live backend branding document is still logo and colors. Icon and splash https URLs are stored on the dashboard until `POST /branding/icon` and `POST /branding/splash` return those fields on `GET /branding`. The signed upload fallback places the file in the store folder that signature already uses. It is not registered as a notification image.
